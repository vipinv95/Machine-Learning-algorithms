...
# Principal Component Analysis (PCA) Algorithm for dimensionality reduction
...
def pca(X = np.array([]), no_dims = 50):
    ...
    N, D = X.shape
    S = np.einsum('jk,jl->lk',X,X)/float(N)
    eig_val_s, eig_vec_s = np.linalg.eig(S)
    sorted_eigv = np.argsort(eig_val_s)
    idx = sorted_eigv[-no_dims:]
    eig_val_s = eig_val_s[idx]
    eig_vec_s = eig_vec_s[:,idx]
    M = eig_vec_s
    Y = np.dot(X,M)
    return Y, M
...
def reconstruction_error(orig = np.array([]), decompressed = np.array([])):
    ...
    error = np.sum(np.square(orig-decompressed))/len(orig)
    return error
