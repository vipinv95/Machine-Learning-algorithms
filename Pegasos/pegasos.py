...
# Pegasos Algorithm - A stochastic gradient based solver for linear SVM
...
def objective_function(X, y, w, lamb):
    ...
    second_part = sum([1-(y[n]*np.dot(w.transpose(),X[n])) if 1-(y[n]*np.dot(w.transpose(),X[n])) > 0 else 0 for n in range(N)])/N
    obj_value = ((lamb/2)*w_norm_squared)+second_part
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        A_t_plus = [a for a in A_t if (ytrain[a]*np.matmul(w.transpose(),Xtrain[a])) < 1]
        eta_t = 1/(lamb*iter)
        s = []
        for ap in A_t_plus:
            s.append(np.array([ytrain[ap]*Xtrain[ap]]))
        su = np.sum(s,0)
        w_t_half = ((1-(eta_t*lamb))*w)+((eta_t/k)*su.transpose())
        if ((sum([wt**2 for wt in w_t_half])**0.5) is not 0):
            w = w_t_half if ((1/(lamb**0.5))/(np.linalg.norm(w_t_half))) > 1 else ((1/(lamb**0.5))/(np.linalg.norm(w_t_half)))*w_t_half
...
def pegasos_test(Xtest, ytest, w, t = 0.):
    ...
    w = np.squeeze(w)
    y_classifier = [-1 if np.dot(w,Xtest[f]) < t else 1 for f in range(len(Xtest))]
    test_acc = sum([1 for yi in range(len(ytest)) if ytest[yi] == y_classifier[yi]])/len(ytest)    
